...
void display_text(const char *text1, const char *text2) {
...
int main() {
    ...
    while (true) {
        ...
        sprintf(str, "%.2f", temperature);  // Converte a temperatura para string com 2 casas decimais
        printf("Temperatura: %s C\n", str);
        sleep_ms(1500);
        display_text("Temperatura", str);  // Exibe a temperatura no display
        if (temperature >= 35.0)
        {
            gpio_put(LEDVM, 1);
            gpio_put(LEDV, 0);
